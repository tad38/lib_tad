...
function is(par_objet, par_type) { return (par_objet instanceof par_type); }
...
function is_objetdom(par_objet) { if (type(par_objet) === "ObjetDomTad") { return true; } else { return false; } }
function is_objetdomtad(par_objet) { if (type(par_objet) === "ObjetDomTad") { return true; } else { return false; } }

// Conversions de différents types de variables
function liste_en_tableau(par_liste) { if (par_liste) { var retour = []; for (i = 0; i < par_liste.length; i++) { retour[i] = par_liste[i]; } return retour; } else { return null; } }
